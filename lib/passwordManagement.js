...
var PasswordManagement = function(db){
        ...
        return new Promise(function (resolve, reject) {
            db.users.firstAsync({email: email}).then(function (user) {
                if (user){
                    ...
                    return reject(appStrings.emailDoesNotExist);
                }
            })
        });
    };
    ...
    var createPasswordReset = function (user) {
        return new Promise(function (resolve, reject) {
            var token = hat();
            db.user.updateOnlyAsync({resetToken: token}, user.id).then(function (result) {
    ...
    module.sendForgotPasswordEmail = function (email) {
        return new Promise(function (resolve, reject) {
            if (!email) {
                return reject(appStrings.needEmail);
            }
            checkIfUserExists(email).then(createPasswordReset).then(emailPasswordReset).then(function () {
                resolve();
            }).done();
        });
    };
    ...
    var findUserByToken = function (args) {
        return new Promise(function (resolve, reject) {
            db.users.firstAsync({resetToken: args.token}).then(function (user) {
                ...
                return reject({message: appStrings.tokenInvalid});
            })
        });
    };
    ...
    module.resetPassword = function (token, password, confirm) {
        return new Promise(function (resolve, reject) {
            if (password !== confirm){
                return reject({message: appStrings.passwordsDoNotMatch})
            }
            var args = {};
            args.token = token;
